...
**Does a pair of transcription-factor motifs form a real composite element, or do
they just share the same accessible DNA?**

`motifdist` analyzes the *spatial relationships* between TF motifs that have
already been mapped to genome coordinates — the seqlet annotation table produced
by a ChromBPNet / TF-MoDISco run. Given where motifs land and which pattern each
belongs to, it answers two questions:

1. **Which motif pairs co-occur more than chance?**
2. **Of those, which have a preferred spacing/orientation** — a genuine
   **composite element** — rather than merely co-habiting a busy regulatory
   region (a **billboard**)?

The reusable, model-independent contribution is **four artifact filters** and
**two distinct null models** that separate real motif grammar from artifacts.
They run identically on any ChromBPNet/MoDISco output, any tissue.

> `motifdist` is **not** a seqlet caller or back-annotator. It *starts* from an
> annotation table. Producing that table lives in the upstream
> [Modisco_Annotation](https://github.com/wgzeng-maker/Modisco_Annotation) repo.
